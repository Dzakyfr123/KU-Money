<template>
  <div class="relative category-select-container">
    <button
      ...
      :disabled="disabled"
      class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent transition flex items-center justify-between disabled:bg-gray-100 disabled:cursor-not-allowed"
    >
      <div class="flex items-center gap-3 flex-1 min-w-0">
        <span v-if="selectedCategory" class="text-2xl shrink-0">
          <i
            ...
            :class="[selectedCategory.icon, 'text-green-600']"
          ></i>
    ...
    >
      <div class="p-2">
        <input
          ...
          class="w-full px-3 py-2 border border-gray-300 rounded-lg mb-2 focus:outline-none focus:ring-2 focus:ring-green-600"
        />
        <div class="space-y-1">
          <button
            ...
            class="w-full px-3 py-2 hover:bg-green-50 rounded-lg transition flex items-center gap-3 text-left"
            :class="{
              'bg-green-100 border-2 border-green-600': selectedCategory?._id === category._id,
            }"
          >
            <span class="text-xl shrink-0">
              <i
                v-if="category.icon && category.icon.startsWith('mdi')"
                :class="[category.icon, 'text-green-600']"
              ></i>
