    ...
    >
      ...
      <nav class="p-4 space-y-2">
        <RouterLink
          ...
          :key="item.path"
          :to="item.path"
          class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors hover:bg-green-50"
          active-class="bg-green-600 text-white hover:bg-green-700"
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
        <div class="mb-3 px-4 py-3 bg-green-50 rounded-lg">
          <p class="text-xs text-gray-600">Status Akun</p>
          <p class="text-sm font-bold text-green-600 uppercase">{{ userStatus }}</p>
        </div>
        <button
          @click="handleLogout"
...
const menuItems = [
  ...
  { path: '/app/complaints', label: 'Keluhan', icon: 'mdi mdi-comment-alert' },
